...
class ERPNextPDFRenamer {
        ...
        <section class="renamer-card">
          ...
          <div class="renamer-view" data-view="upload">
            <div class="renamer-dropzone" tabindex="0" role="button" aria-label="Choose one multi-page PDF">
              ...
              <small>PDF only · even number of pages · maximum 100 MB</small>
            </div>
  ...
  select_file(file) {
    ...
    if (file.size > 100 * 1024 * 1024) return this.show_error(__("The PDF must be 100 MB or smaller."));
    this.file = file;
    this.queue = [{
      name: __("Pages 1–2"),
      status: "waiting",
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onload = () => {
        ...
        try {
          ...
          const messages = {
            413: __("The web server rejected this upload as too large (HTTP 413). Set its request limit above 100 MB."),
